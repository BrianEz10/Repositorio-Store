    ...
    <article className='border border-[#5b403e]/20 bg-[#1a1a1a] p-8'>
      <div className='mb-8 flex flex-col gap-4 border-b border-[#5b403e]/20 pb-6 md:flex-row md:items-center md:justify-between'>
        <div>
          ...
          <h2 className={`mt-2 text-3xl font-black uppercase ${STATUS_COLORS[order.estado_codigo]?.split(' ')[0] ?? 'text-white'}`}>
      ...
      <div className='space-y-4'>
        ...
        {order.detalles.length > 3 && (
          <p className='text-center text-xs text-[#ab8986]'>
      ...
      <div className='mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
        <Link
          to={`/orders/${order.id}`}
          className='border border-[#ffb3ae]/40 px-4 py-2 text-center text-xs font-bold uppercase tracking-[0.2em] text-[#ffb3ae] transition hover:border-[#ffb3ae] hover:bg-[#ffb3ae] hover:text-black'
        >
      ...
      {CANCELABLE.includes(order.estado_codigo) && (
        <div className='mt-6 border-t border-[#5b403e]/20 pt-6'>
          {showConfirm ? (
